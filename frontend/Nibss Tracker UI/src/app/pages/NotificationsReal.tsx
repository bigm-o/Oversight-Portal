...
import { Bell, Clock, Rocket, ShieldAlert } from 'lucide-react';
import { Badge } from '@/app/components/ui/badge';

export function NotificationsReal() {
  return (
    <div className="min-h-[80vh] flex flex-col items-center justify-center p-4 space-y-8 animate-in fade-in zoom-in duration-700">
      <div className="relative group">
        ...
        <div className="relative w-32 h-32 bg-white dark:bg-slate-900 rounded-3xl shadow-2xl border border-slate-200 dark:border-slate-800 flex items-center justify-center">
          <Bell className="w-16 h-16 text-green-600 animate-bounce" />
          <div className="absolute -top-2 -right-2 w-8 h-8 bg-rose-600 rounded-full border-4 border-white dark:border-slate-900 flex items-center justify-center">
      ...
      <div className="text-center space-y-4 max-w-2xl">
        ...
        <p className="text-xl text-muted-foreground font-medium">
          The central nervous system of IT Governance is currently being engineered.
        </p>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6 w-full max-w-4xl mt-8">
        {[
          { icon: Bell, title: 'Real-time Alerts', desc: 'Instant desktop and mobile notifications for SLA breaches.' },
          { icon: Clock, title: 'Smart Digest', desc: 'AI-powered daily summaries of critical governance movements.' },
          { icon: Rocket, title: 'Automated Escalation', desc: 'Direct alerts to engineering leads when stagnation is detected.' }
        ].map((feat, i) => (
